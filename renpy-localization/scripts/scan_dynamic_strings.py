#!/usr/bin/env python3
"""Audit dynamic and persistent-backed Ren'Py text for localization risks."""
...
PERSISTENT_I18N_ASSIGN_RE = re.compile(
    r"^\s*(?:(?:default|define)\s+|\$\s*)?"
    r"persistent\.[A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*\s*=.*\b(?:_p|_)\s*\("
)
CALL_RE = re.compile(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fail-on-persistent-i18n", action="store_true")
...
def scan_persistent_i18n(root: Path) -> list[Candidate]:
    risks: list[Candidate] = []
    for path in root.rglob("*.rpy"):
        for number, line in enumerate(path.read_text(encoding="utf-8-sig").splitlines(), 1):
            if PERSISTENT_I18N_ASSIGN_RE.match(line):
                risks.append(
                    Candidate(
                        line.strip(),
                        path,
                        number,
                        "translated text stored in persistent data",
                    )
                )
    return risks
...
def main() -> int:
    ...
    persistent_i18n_risks = scan_persistent_i18n(source_root)
    ...
    print(f"persistent_i18n_risks={len(persistent_i18n_risks)}")
    ...
    for candidate in persistent_i18n_risks:
        print(
            f"PERSISTENT_I18N_RISK\t{candidate.path}:{candidate.line}\t"
            f"{candidate.reason}\t{candidate.text}"
        )

    if (
        bad_targets
        or (args.fail_on_uncovered and uncovered)
        or (args.fail_on_persistent_i18n and persistent_i18n_risks)
    ):
        return 1
    return 0
